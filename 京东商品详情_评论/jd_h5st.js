...
function getEncryptedCollectInfo() {
            collectConfig = {
    fp: {
        language: "language",
        userAgent: "userAgent",
    ...
    getFpExcludeOptions: function(l) {
        ...
        if ("fast" == l)
            ...
            g[collectConfig.env.canvas] || (f.ca = n_execute("getCanvasInfo") || {});
            g[collectConfig.env.browserMode] || (f.m = n_execute("getBrowserMode") || {});
            g[collectConfig.env.fonts] || (f.fo = n_execute("getSupportFonts") || []);
            g[collectConfig.env.feature] || (f.n = n_execute("getFeature") || {});
            g[collectConfig.env.plugins] || (f.p = n_execute("getPlugins") || []);
            g[collectConfig.env.position] || (f.w = n_execute("getPositionInfo") || {});
            g[collectConfig.env.screen] || (f.s = n_execute("getScreenInfo") || {});
            g[collectConfig.env.color] || (f.sc = n_execute("getColorRgb") || {});
            g[collectConfig.env.storeCheck] || (f.ss = n_execute("getStoreCheck") || {});
            f.tz = d.getTimezoneOffset();
            f.lil = "";
            f.wil = "";
            f.ts.deviceEndTime = (new Date).getTime();
            false && console.log("collect env data :", f);
            return  a_token(f)
        }
window.PSign = new ParamsSign({
    appId: "fb5df"
})
function gettoken(sku){
    g = {
        "pin": "",
        "oid": "",
        "bizId": "PCSX-HQJG",
        "fc": "4TGUABOZWY7C3HHN3BNBKXK3POLEATE4KJH2KYI4BEMGZWMW4F6DFHWPQXYBCR577YNJMC2OOS3ZOYZREJBCOC6NBY",
        "mode": "strict",
        "p": "s",
        "fp": "7db64aa73124ec73f90751b5c8a7982c",
        "ctype": 1,
        "v": "3.1.1.0",
        "f": "1",
        "o":`item.jd.com/${sku}.html`,
        "qs": "",
        "jsTk": "jdd034TGUABOZWY7C3HHN3BNBKXK3POLEATE4KJH2KYI4BEMGZWMW4F6DFHWPQXYBCR577YNJMC2OOS3ZOYZREJBCOC6NBYAAAAMLCKN56UIAAAAACTPUQK4RONYC7EX",
        "qi":''
    }
    return JSON.stringify({"a": a_token(g), "d": getEncryptedCollectInfo()})
}

app.get("/get", function (req,res){
    r =JSON.parse(req.query.text)
    info = JSON.parse(JSON.stringify(r));
    info.body = CryptoJS.SHA256(info.body).toString()
    window.PSign.sign(info).then(function (e) {
        res.send(JSON.stringify({'h5st': encodeURI(e.h5st)}))
        console.log(encodeURI(e.h5st))
    })
...
// 运行项目创建服务
//  http://127.0.0.1/get?text
//  生成h5st格式
// '{"appid": "pc-item-soa", "functionId": "pc_detailpage_wareBusiness", "client": "pc", "clientVersion": "1.0.0", "t": 1696497443, "body": "{\"skuId\":10026592976820,\"cat\":\"9987,653,655\",\"area\":\"17_1381_50712_62966\",\"shopId\":\"1000000127\",\"venderId\":1000000127,\"paramJson\":\"{\\\"platform2\\\":\\\"1\\\",\\\"specialAttrStr\\\":\\\"p0ppppppppp2p1ppppppppppppp3p\\\",\\\"skuMarkStr\\\":\\\"00\\\"}\",\"num\":\"1\",\"bbTraffic\":\"\"}"}'
